...
- **Intelligent Scaling**: Auto-recommends question count (25-250 range based on scope and content)
...
|----------|---------|---------|
| **Scope** | Explicit files / Chapter X / Part Y / Chapter X from Part Y | Explicit files |
| **Question count** | Accept recommendation / Override with custom | Auto-recommend based on content |
...
1. DISCOVER SCOPE
   ...
   └── Confirm with user: "Found 12 lessons in Chapter 5. Proceed?"
...
3. CALIBRATE
   └── Recommend question_count based on content density + type
   └── difficulty_profile = analyze_complexity(content)
   └── User accepts recommendation or overrides
...
6. VALIDATE & REMEDIATE
   └── Run structure/distribution checks (existing)
   ...
   └── See references/validation-rules.md and references/bias-detection-guide.md
