...
"Model capability: The agent isn't smart enough to maintain consistency",
"Position sensitivity: The liability criteria are in Zone 2 of CLAUDE.md",
"Token budget: Context utilization is too high for reliable recall"
],
correctOption: 0,
...
options: [
"Compression is slower than deletion for noise reduction",
"Compressed noise is still noise—it consumes instruction budget without providing signal, just more efficiently",
"20 lines is still too many for Zone 2",
"Compression loses important context that might be needed later"
],
correctOption: 1,
...
options: [
"The agent needs a larger context window to hold all paper information",
"Context isolation failure: papers with similar topics are conflating in the shared context space",
"The vector database is returning wrong chunks",
"The agent's training data has gaps in academic citation"
],
correctOption: 1,
...
options: [
"/compact Keep everything important",
"/compact Preserve decisions, discard exploration",
...
"/compact Focus on code, remove discussions"
],
correctOption: 2,
...
options: [
"Parallel execution is inherently unstable for complex analysis",
"8,000 tokens per subagent exceeds the safe return threshold",
"The subagents lacked shared constraints, so they made incompatible assumptions about the contract's baseline terms",
"The orchestrator's context window is too small to synthesize three analyses"
],
correctOption: 2,
...
options: [
"Zone 1 placement only helps at session start; mid-session recall follows different attention patterns",
"55% utilization is too high for brand voice retention",
"The conversation history has accumulated enough noise to dilute even Zone 1 content",
"The guidelines need to be restated every 10 messages"
],
correctOption: 2,
...
options: [
"The vector database needs more memories to improve diversity",
"Increase top_k to return more results per query",
"Deduplication layer: track (memory_id, thinking_hash) pairs with TTL to skip recently-injected memories",
"Use UserPromptSubmit instead of PreToolUse for more stable queries"
],
correctOption: 2,
...
options: [
"Stateful handoff with explicit 'ignore these sections' markers",
"Have Agent A delete its exploration before handoff",
"Clean context pattern: Agent A returns only a structured summary; Agent B receives fresh context with just the summary",
"Larger context window to accommodate both exploration and final work"
],
correctOption: 2,
...
"Whether the standards file was recently modified",
"Whether to add more detailed examples to Zone 1"
],
correctOption: 0,
...
options: [
"claude --continue to resume the full session context",
"/resume to pick from recent sessions and select this one",
"Start fresh session, read progress file to reconstruct state, reference old session only if specific details needed",
"/compact the old session aggressively then continue"
],
correctOption: 2,
...
"15 patterns is too many for reliable recall regardless of position",
"The patterns need more detailed explanations",
"Zone 1 content should be moved to a separate rules file"
],
correctOption: 0,
...
options: [
"Restart the session with a more constrained prompt",
"Add '/compact Focus on authentication only' and continue",
"Memory injection via PreToolUse hook: re-inject the original task scope when the agent's thinking diverges",
"Add 'NEVER refactor without explicit permission' to CLAUDE.md Zone 1"
],
correctOption: 2,
...
options: [
"The reference should be in Zone 3, not Zone 2",
"The reference lacks a trigger condition—it doesn't specify WHEN to read the file",
"External file references don't work reliably across sessions",
"The checklist file is too long for the agent to process"
],
correctOption: 1,
...
options: [
"More detailed rationale for the TypeScript decision",
"Session initialization protocol: the agent isn't reading the progress file at session start",
"The decision should be in CLAUDE.md instead of the progress file",
"Links to the Session 2 conversation where the decision was made"
],
correctOption: 1,
...
options: [
"Manual curation is always better than automated handoff",
"Agent C works better with less context—the automated pipeline is passing too much raw process alongside the summary",
"Copy-paste introduces beneficial formatting changes",
"Agent B's automated summary is lower quality than what you paste"
],
correctOption: 1,
...
options: [
"The agent gets 'tired' in longer sessions",
"Quality degradation at 70% is the U-shaped attention curve hitting its middle trough",
"The 70% threshold triggers automatic compaction which loses important context",
"API rate limiting kicks in after 40 turns"
],
correctOption: 1,
...
"400 lines exceeds the maximum file size Claude can read",
"The file should be a vector database, not markdown",
"Learned preferences should be in CLAUDE.md instead of a separate file"
],
correctOption: 0,
...
options: [
"Sequential execution is inherently slower",
"Later reviewers are influenced by earlier reviewers' framing, losing independent perspective",
"3,000 tokens per reviewer accumulates to too much context",
"The orchestrator synthesis logic doesn't handle sequential input"
],
correctOption: 1,
...
options: [
"Model inconsistency in following instructions",
"52% is high enough for early attention degradation in long conversations",
"The convention instruction is in the middle 80% of CLAUDE.md (Zone 2) where recall drops",
"The agent needs explicit reminders every 10 turns"
],
correctOption: 2,
...
options: [
"System prompts have a maximum size limit",
"System prompts can't be updated mid-session; PreToolUse enables dynamic injection based on evolving needs",
"Memories in system prompts cost more tokens",
"System prompts are visible to users which is a security concern"
],
correctOption: 1,
...
options: [
"Increase summary length to 2,000 tokens",
"Pass full analysis instead of summary",
"Restructure summary format: 'Summary + Supporting Evidence + Caveats' to ensure nuances survive compression",
"Have the subagent flag when important nuances might be lost"
],
correctOption: 2,
...
options: [
"Redundant noise: Claude knows security best practices from training",
...
"Stale noise: Security best practices have changed since Claude's training",
"Position noise: The instruction is in Zone 2"
],
correctOption: 1,
...
options: [
"Single long session with aggressive compaction",
"Fresh sessions daily with comprehensive progress files",
"Shared memory layer (progress file + vector DB) with stateless subagents for specific analyses",
"Multiple sessions using --continue to maintain thread"
],
correctOption: 2,
...
options: [
"The progress file should link to the original conversation",
"Session 2's learning wasn't captured as transferable pattern—it was recorded as event, not rule",
"Conflict-of-interest detection requires real-time memory injection",
"The agent needs better COI detection training"
],
correctOption: 1,
...
options: [
"CLAUDE.md at 12% is too high",
"Tool definitions at 15% suggests too many tools enabled",
"Reserve at 5% is dangerously low—no buffer for unexpected file reads or complex responses",
"Message history at 45% is normal for mid-session"
],
correctOption: 2,
...
options: [
"Run linter less frequently",
"Increase context window allocation for tool outputs",
"Summarize linter output: 'Style issues: 3 indentation, 2 naming conventions in lines 45, 78, 92' instead of full verbose output",
"Move linting to a separate pre-processing step"
],
correctOption: 2,
...
options: [
"Whether the rules are formatted as questions vs commands",
"Whether the agent model version has changed",
"Whether the rules are truly actionable—can Claude actually follow them, or are they aspirational?",
"Whether Zone 1 needs to be expanded"
],
correctOption: 2,
...
options: [
"Reduce subagent count to 2",
"Increase subagent context allocation",
...
options: [
"Stronger wording in the original instruction",
"Adding the instruction to Zone 1 and Zone 3 for redundancy",
"Workflow-level verification: a post-generation check that validates approval status before marking 'ready'",
"Memory injection of the approval requirement at turn 38"
],
correctOption: 2,
...
options: [
"Context engineering techniques need to be re-applied periodically",
"The underlying model was updated and changed behavior",
...
"The techniques have a natural expiration and need replacement"
],
correctOption: 2,
