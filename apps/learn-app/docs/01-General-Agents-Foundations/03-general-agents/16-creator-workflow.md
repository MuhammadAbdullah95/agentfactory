...
- **Lesson 09 (Subagents)**: Each session is like a subagent with a specific task—research in one, drafting in another, review in a third.
...
- **Lesson 09 (Subagents)**: Plan is a built-in subagent that researches your codebase. The reviewer is effectively another subagent with a different role.
...
- **Lesson 13 (Hooks)**: You learned the PostToolUse pattern. This is a production example that prevents a common frustration.
...
- **Lesson 12 (Settings Hierarchy)**: Team-shared settings in `.claude/settings.json` ensure consistency across team members.
...
| Parallel sessions (3-5)     | Team        | L01 + L09      | Isolate sessions for true parallelism                        |
| Workspace shortcuts         | Team        | Workspace      | Quick switching between workstreams (aliases, tabs)          |
| Claude-reviews-Claude       | Team        | L09            | Fresh context catches blind spots                            |
| Plan Mode first             | Team + Docs | L09            | Always plan before executing non-trivial tasks               |
| Re-plan when stuck          | Team        | L09            | Switch back to Plan Mode when things go sideways             |
...
| "use subagents" directive   | Team        | L09            | Append to throw more compute at problems                     |
| Autonomous problem solving  | Team        | MCP + L09      | Give Claude the problem, not the solution                    |
...
| Verification loops          | Team + Docs | L10 + L13      | Give Claude tools to verify its own work                     |
...
| PostToolUse hooks           | Team        | L13            | Automate consistent formatting or checks                     |
| `/permissions` over skip    | Team        | L12            | Pre-allow safe operations, share with team                   |
...
| Opus 4.5 choice             | Team        | L12            | Optimize for total iteration time, not speed                 |
