...
A script becomes a real tool when you can run it by name from any directory. Shell aliases stored in config files (`.bashrc` or `.zshrc`) make commands permanent across terminal sessions. The test: close your terminal, open a new one, and the command still works.

### Key Mental Models

- **Script vs tool gap**: A script in a random folder requires remembering the full path every time. A tool is available everywhere by name. The difference is organization + aliases + config persistence.
...
- Leaving scripts scattered across project folders instead of organizing them in one place like `~/tools`

### Connections

- **Builds on**: sum-expenses.py from Lesson 3
- **Leads to**: Tax categorization with regex (Lesson 5)
