...
A session is your conversation with the database. You open it (context manager), make changes or ask questions, and close it. Creating records uses `session.add()` + `session.commit()`; reading uses `session.query()` with filtering, ordering, and limiting.
...
- **Database-side filtering over Python loops**: Always push filtering to the database with `.filter()` instead of loading all records and looping in Python. The database returns only matching rows, which is vastly more efficient.
...
- Query all: `session.query(Model).all()` returns a list
- Query with filter: `session.query(Model).filter(Model.field == value).first()` returns one object or None
- Multiple filters (AND): pass multiple conditions to `.filter()` separated by commas
