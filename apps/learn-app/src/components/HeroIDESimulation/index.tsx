...
function highlightCode(code: string): React.ReactNode[] {
  ...
  return lines.map((line, lineIndex) => {
    ...
    while (remaining.length > 0) {
      ...
      if (stringMatch) {
        tokens.push(
          <span key={tokenIndex++} className="text-[#98c379]">
            {stringMatch[0]}
          </span>,
        );
      ...
      if (decoratorMatch) {
        tokens.push(
          <span key={tokenIndex++} className="text-[#e5c07b]">
            {decoratorMatch[0]}
          </span>,
        );
      ...
      if (commentMatch) {
        tokens.push(
          <span key={tokenIndex++} className="text-[#5c6370] italic">
            {commentMatch[0]}
          </span>,
        );
        remaining = "";
      ...
      for (const keyword of PYTHON_KEYWORDS) {
        ...
        if (keywordRegex.test(remaining)) {
          tokens.push(
            <span key={tokenIndex++} className="text-[#c678dd]">
              {keyword}
            </span>,
          );
      ...
      if (funcMatch) {
        tokens.push(
          <span key={tokenIndex++} className="text-[#61afef]">
            {funcMatch[0]}
          </span>,
        );
      ...
      if (typeMatch) {
        tokens.push(
          <span key={tokenIndex++} className="text-[#56b6c2]">
            {typeMatch[0]}
          </span>,
        );
      ...
      if (numberMatch) {
        tokens.push(
          <span key={tokenIndex++} className="text-[#d19a66]">
            {numberMatch[0]}
          </span>,
        );
      ...
      // Default (light gray)
      tokens.push(
        <span key={tokenIndex++} className="text-[#abb2bf]">
          {remaining[0]}
        </span>,
      );
...
export function IDEShowcaseSection() {
  ...
  const staticCode =
    pythonSequence.steps.find((s) => s.type === "code")?.content || "";
  ...
  // Memoize expensive syntax highlighting - only re-compute when code changes
  const highlightedCode = useMemo(() => {
    if (!displayedCode) return null;
    return highlightCode(displayedCode);
  }, [displayedCode]);
              ...
              <div className="code-scrollbar flex-1 bg-[#1e1e1e] font-mono text-[13px] leading-relaxed py-4 overflow-y-auto min-h-0">
                {highlightedCode ? (
                  ...
                  <div className="flex min-h-[1.5em] pl-4">
                    <span className="w-[45px] text-right pr-4 text-[#4b5263] text-[13px]">
                      1
                    </span>
