...
describe('getExercise', () => {
  ...
  it('returns config for all four exercises', () => {
    const ids = ['ch3-basics', 'ch3-skills', 'ch3-plugins', 'ch3-agent-teams'];
    for (const id of ids) {
  ...
  it('each exercise has a description', () => {
    ...
    for (const [id, config] of Object.entries(exercises)) {
      expect(config.description, `${id} should have a description`).toBeDefined();
...
describe('listExercises', () => {
  ...
  it('mutations to returned object do not affect registry', () => {
    const exercises = listExercises();
    (exercises as Record<string, unknown>)['hacked'] = { repo: 'evil', releaseTag: 'v1' };
