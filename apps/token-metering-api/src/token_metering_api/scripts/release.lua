-- release.lua: Remove reservation from sorted set on LLM failure (v5)
...
--
-- Returns: {status, released_tokens}
--   status = 1: reservation found and released
--   status = 0: reservation not found (may have expired or already released)
--
-- Note: This is idempotent - releasing a non-existent reservation succeeds (FR-063)
...
-- Find and remove the member with matching request_id - FR-038
